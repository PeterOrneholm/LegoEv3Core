...
namespace LegoEv3Core.Communication
{
    ...
    public class UsbCommunication : ICommunication
    {
        ...
        public Task ConnectAsync()
        {
            ...
            Task t = Task.Factory.StartNew(async () =>
            {
                while (!_tokenSource.IsCancellationRequested)
                {
                    ...
                    ReportReceived?.Invoke(this, new ReportReceivedEventArgs { Report = report });
                }
        ...
        internal void FindEv3()
        {
            ...
            var hDevInfo = HidImports.SetupDiGetClassDevs(ref guid, null, IntPtr.Zero, HidImports.DIGCF_DEVICEINTERFACE | HidImports.DIGCF_PRESENT);

            // create a new interface data struct and initialize its size
            var diData = new HidImports.SP_DEVICE_INTERFACE_DATA();
            ...
            while (HidImports.SetupDiEnumDeviceInterfaces(hDevInfo, IntPtr.Zero, ref guid, index, ref diData))
            {
                ...
                HidImports.SetupDiGetDeviceInterfaceDetail(hDevInfo, ref diData, IntPtr.Zero, 0, out var size, IntPtr.Zero);

                // create a detail struct and set its size
                var diDetail = new HidImports.SP_DEVICE_INTERFACE_DETAIL_DATA
                {
                ...
                if (HidImports.SetupDiGetDeviceInterfaceDetail(hDevInfo, ref diData, ref diDetail, size, out size, IntPtr.Zero))
                {
                    ...
                    var attrib = new HidImports.HIDD_ATTRIBUTES();
                    ...
                    if (HidImports.HidD_GetAttributes(_handle.DangerousGetHandle(), ref attrib))
                    {
                        ...
                        if (attrib.VendorID == Vid && attrib.ProductID == Pid)
                        {
                            ...
                            if (!HidImports.HidD_GetPreparsedData(_handle.DangerousGetHandle(), out var preparsedData))
                                throw new Exception("Could not get preparsed data for HID device");

                            if (HidImports.HidP_GetCaps(preparsedData, out var caps) != HidImports.HIDP_STATUS_SUCCESS)
